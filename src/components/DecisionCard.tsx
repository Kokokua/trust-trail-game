...
export function DecisionCard({ room, onChoose, onDismiss, currentPick, style, className }: Props) {
  const [asked, setAsked] = useState(false);

  return (
    <div
      style={style}
      className={`animate-pop w-[min(22rem,90vw)] max-h-[min(34rem,85vh)] overflow-y-auto rounded-2xl border border-border bg-popover p-4 shadow-[var(--shadow-card)] ${className ?? ""}`}
    >
      ...
      <div className="mt-3 space-y-2">
        <button
          ...
          className={`w-full rounded-xl border p-3 text-left transition-all hover:-translate-y-0.5 hover:border-primary hover:shadow-[var(--shadow-soft)] ${
            currentPick === "care"
      ...
      >
        {currentPick ? "Keep current setting" : "Not right now"}
      </button>
    </div>
  );
}
